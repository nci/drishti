...
GlewInit::initialise()
{
  if (!glew_initialised)
	...
	// Some Intel compatibility-profile drivers expose a valid OpenGL 4.5
	// context while GLEW's aggregate 4.5 flag stays false because an unused
	// 4.5 entry point is unavailable. The renderer needs GLSL 4.50, not every
	// OpenGL 4.5 direct-state-access function.
      if (!GLEW_VERSION_4_5 && !contextVersionAtLeast(4, 5))
	{
	  QMessageBox::critical(0, "OpenGL",
				QStringLiteral("Drishti requires OpenGL 4.5 or newer.\n\n%1")
				.arg(contextInfo));
	  return false;
	}
	...
	{
	  QMessageBox::critical(0, "OpenGL",
				QStringLiteral("Drishti requires an OpenGL compatibility profile.\n\n%1")
				.arg(contextInfo));
	  return false;
	}
#else
	...
	{
	  QMessageBox::critical(0, "OpenGL",
				QStringLiteral("Drishti requires framebuffer objects.\n\n%1")
				.arg(contextInfo));
	  return false;
	}
