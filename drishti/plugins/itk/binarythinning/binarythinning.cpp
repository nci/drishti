...
QStringList
BinaryThinning::registerPlugin()
{
  QStringList regString;

  regString << "Skeletonization";
...
void BinaryThinning::setLodFileManager(VolumeFileManager *l) { m_lodFileManager = l; }
void BinaryThinning::setClipInfo(QList<Vec> cp, QList<Vec> cn) { m_clipPos = cp; m_clipNormal = cn; }
void BinaryThinning::setCropInfo(QList<CropObject> c) { m_crops = c; }
void BinaryThinning::setSamplingLevel(int s) { m_samplingLevel = s; }
void BinaryThinning::setDataLimits(Vec dmin, Vec dmax) { m_dataMin = dmin; m_dataMax = dmax; }
void BinaryThinning::setVoxelScaling(Vec v) { m_voxelScaling = v; }
void BinaryThinning::setPreviousDirectory(QString d) { m_previousDirectory = d; }
void BinaryThinning::setLookupTable(int ls, QImage li) { m_lutSize = ls; m_lutImage = li; }

void BinaryThinning::setPathInfo(QList<PathObject> c)
{
  m_paths.clear();
...
void
BinaryThinning::setPruneData(int lod,
...
//-------------------------------
//-------------------------------
Q_EXPORT_PLUGIN2(binarythinning, BinaryThinning);
