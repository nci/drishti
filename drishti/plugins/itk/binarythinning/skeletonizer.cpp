  ...
  try
  ...
  catch (const std::exception& error)
    {
      QMessageBox::critical(0, "Error",
                            QString("Cannot create the binary skeleton.\n%1")
                            .arg(QString::fromLocal8Bit(error.what())));
    }
  catch ( ... )
    {
      QMessageBox::critical(0, "Error", "Cannot create the binary skeleton because allocation or filter execution failed.");
    }
				   ...
				   int chan)
{
  if (m_voxelType != VolumeFileManager::_UChar &&
    ...
    throw std::runtime_error(
        "Binary thinning supports only unsigned 8-bit and 16-bit volumes");
  const int bpv = (m_voxelType == VolumeFileManager::_UShort ? 2 : 1);
  if (m_nX <= 0 || m_nY <= 0 || m_nZ <= 0)
    throw std::runtime_error("Binary-thinning dimensions must be positive");

  qint64 voxelCount = m_nX;
  if (voxelCount > std::numeric_limits<qint64>::max()/m_nY)
    throw std::runtime_error("Binary-thinning dimensions overflow addressable memory");
  voxelCount *= m_nY;
  ...
  if (static_cast<quint64>(voxelCount) > std::numeric_limits<size_t>::max() ||
    ...
    throw std::runtime_error("Binary-thinning exceeds addressable memory");
  ...
  if (planeBytes64 > sourcePlaneBytes64 ||
    ...
    throw std::runtime_error("Source slice is smaller than the requested thinning plane");

  const auto checkedPlaneIndex = [=](int row, int column) -> size_t
    {
      if (row < 0 || row >= m_nY || column < 0 || column >= m_nZ)
        throw std::runtime_error("Thinning plane index is out of bounds");
      const qint64 index = static_cast<qint64>(row)*m_nZ + column;
      if (index < 0 || index >= planeVoxels)
        throw std::runtime_error("Thinning plane index exceeds its buffer");
  ...
  if (fp.write((char*)&vt, 1) != 1 ||
      ...
      fp.write(tdata, voxelCount) != voxelCount)
    {
      const QString detail = fp.errorString();
      fp.cancelWriting();
      throw std::runtime_error(
          QString("Cannot write complete skeleton volume: %1")
          .arg(detail).toStdString());
    }
  if (!fp.commit())
    throw std::runtime_error(
        QString("Cannot commit skeleton volume: %1")
