...
QStringList
ConnectedComponent::registerPlugin()
{
  QStringList regString;

  regString << "Connected Component Labeling";

  return regString;
}

void
ConnectedComponent::init()
{
  m_pvlFileManager = 0;
  m_lodFileManager = 0;
  m_clipPos.clear();
...
void ConnectedComponent::setPvlFileManager(VolumeFileManager *p) { m_pvlFileManager = p; }
void ConnectedComponent::setLodFileManager(VolumeFileManager *l) { m_lodFileManager = l; }
void ConnectedComponent::setClipInfo(QList<Vec> cp, QList<Vec> cn) { m_clipPos = cp; m_clipNormal = cn; }
void ConnectedComponent::setCropInfo(QList<CropObject> c) { m_crops = c; }
void ConnectedComponent::setSamplingLevel(int s) { m_samplingLevel = s; }
void ConnectedComponent::setDataLimits(Vec dmin, Vec dmax) { m_dataMin = dmin; m_dataMax = dmax; }
void ConnectedComponent::setVoxelScaling(Vec v) { m_voxelScaling = v; }
void ConnectedComponent::setPreviousDirectory(QString d) { m_previousDirectory = d; }
void ConnectedComponent::setLookupTable(int ls, QImage li) { m_lutSize = ls; m_lutImage = li; }

void ConnectedComponent::setPathInfo(QList<PathObject> c)
{
  m_paths.clear();
...
void
ConnectedComponent::setPruneData(int lod,
