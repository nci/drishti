...
{
  m_MultiScaleVesselnessFilter->AddObserver(event, cmd);
  m_HessianFilter->AddObserver(event, cmd);
  m_EigenVectorMatrixAnalysisFilter->AddObserver(event, cmd);
  m_DiffusionTensorImage->AddObserver(event, cmd);
  return Superclass::AddObserver(event, cmd);
}
