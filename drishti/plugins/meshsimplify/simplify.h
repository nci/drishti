...
namespace Simplify
{
	...
	void simplify_mesh(QTextEdit *meshLog,
			   int target_count,
	...
	//------------------------------------
	// convert ply data to internal format
	//------------------------------------
	void load_ply(int nverts, int nfaces,
		      PlyVertex **vlist, PlyFace **flist)
	{
	  vertices.clear();
	  triangles.clear();
	  ...
	  for(int i=0; i<nfaces; i++)
	    {
	      Triangle t;
	      t.v[0] = flist[i]->verts[0];
	      t.v[1] = flist[i]->verts[1];
	...
	//Option : Load OBJ
	void load_obj(char* filename, bool process_uv=false){
		vertices.clear();
		triangles.clear();
