...
void
VolumeSingle::getVolume(uchar *lut,
			QList<Vec> clipPos,
			 ...
			 QList<PathObject> paths)
{
	...
	for(int x=minx; x<=maxx; x++)
	  {
	    ...
	    // we don't want to scale it before pruning
	    {
	      ...
	      ok = (prune[ppk*gridy*gridx + ppj*gridx + ppi] > 0);
	    }
  ...
  QString str;
  str = QString("Non-Zero Voxels : %1\n").arg(m_nonZeroVoxels);
  str += QString("Non-Zero Voxel Volume : %1 %2\n").\
         arg(voxvol).\
         arg(pvlInfo.voxelUnitStringShort());

  float totvol = (maxx-minx+1);
  totvol *= (maxy-miny+1);
  totvol *= (maxz-minz+1);
  float percent = (float)m_nonZeroVoxels/totvol;
  percent *= 100.0;
