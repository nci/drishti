...
DColorWheel::setColor(QColor col)
{
  ...
  if (m_saturation < 0.01)
    {
      if (m_value >= 0.5)
	m_value = 2.0*m_value-1.0;
      else
	{
	  m_saturation = 1.0-2.0*m_value;
	  m_value = 0;
	}
    }
}
...
DColorWheel::getColor()
{
  float sat, val;
  int colx, coly;
  colx = ceil(m_saturation*(m_colorGridSize)) - 1;
  coly = ceil(m_value*(m_colorGridSize)) - 1;
  colx = qMax(0, qMin(colx, m_colorGridSize-1));
  coly = qMax(0, qMin(coly, m_colorGridSize-1));
  if (colx == 0 || coly == 0)
    {
      sat = 0;
      val = 0;
      if (coly == 0)
	val = (m_colorGridSize-1-colx)/(float)(2*m_colorGridSize-2);
      else
	val = (m_colorGridSize-1+coly)/(float)(2*m_colorGridSize-2);
    }
  else
    {
      if (m_hue < 0)
	{
	  val = (colx*m_colorGridSize + coly);
	  val/=(m_colorGridSize*m_colorGridSize);
	  sat = 0;
	}
      else
	{
	  sat=(float)colx/(m_colorGridSize-1);
	  val=(float)coly/(m_colorGridSize-1);
	}
    }
...
DColorWheel::paintEvent(QPaintEvent *event)
{
  ...
  int i,j;
  float width = (float)m_innerRect.width()/m_colorGridSize;
  float height = (float)m_innerRect.height()/m_colorGridSize;
  // plot gray values
  for(i=0; i<2*m_colorGridSize-1; i++)
    {
      int x, y;
      float v;
      if (i < m_colorGridSize)
	{
	  x = m_colorGridSize-1-i;
	  y = 0;
	}
      else
	{
	  x = 0;
	  y = i-m_colorGridSize+1;
	}
      v = i/(float)(2*m_colorGridSize-2);
      QColor col;
  ...
  // plot color values
  for(i=1; i<m_colorGridSize; i++)
  for(j=1; j<m_colorGridSize; j++)
    {
      float s,v;
      if (m_hue < 0)
	{
	  v = (i*m_colorGridSize + j);
	  v/=(m_colorGridSize*m_colorGridSize);
	  s = 0;
	}
      else
	{
	  s = i/(float)(m_colorGridSize-1);
	  v = j/(float)(m_colorGridSize-1);
	}
      QColor col;
