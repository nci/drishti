...
void
KeyFrameEditor::setPrayogMode(bool m)
{
  m_PrayogMode = m;
  ...
  if (m_PrayogMode)
    {
      ...
      m_tickHeight = 5;
      m_imgSpacer = 5;
      m_editorHeight = 10 + m_lineHeight + m_tickHeight +
                            m_imgSpacer + m_imgSize + 10;

      for(int i=0; i<m_fno.size(); i++)
	{
	  m_fno[i] = m_minFrame + i*(m_maxFrame-m_minFrame)/(m_fno.size()-1);
	}
  ...
  QWidget(parent)
{
  ...
  // set font size for the entire widget
...
//  QString ss;
//  ss = "QPushButton { border: 2px solid silver; background:ghostwhite; border-radius : 10px; }";
//  m_set->setStyleSheet(ss);
//  m_remove->setStyleSheet(ss);
//  m_set->setFont(QFont("Helvetica", 12));
//  m_remove->setFont(QFont("Helvetica", 12));
  
  m_plus = new QPushButton(QPixmap(":/images/zoomin.png"), "Z");
  m_minus = new QPushButton(QPixmap(":/images/zoomout.png"), "z");
  m_lock = new QPushButton(QPixmap(":/images/unlock.png"), "z");
  m_play = new QPushButton(QPixmap(":/images/animation-start.png"), ">");
  m_reset = new QPushButton(QPixmap(":/images/animation-reset.png"), "|<");
 

//  //ss = "QPushButton { border: 2px solid #8f8ffa; background:ivory; border-radius : 15px; }";
//  ss = "QPushButton { border: 2px solid silver; background:ivory; border-radius : 15px; }";
//  m_plus->setStyleSheet(ss);
//  m_minus->setStyleSheet(ss);
//  m_lock->setStyleSheet(ss);
//  m_play->setStyleSheet(ss);
//  m_reset->setStyleSheet(ss);
  ...
  m_p0 = QPoint(10+m_imgSize/2, m_lineHeight);
  m_p1 = QPoint(m_imgSize, m_lineHeight);
  ...
  connect(m_lock, SIGNAL(pressed()),
	  this, SLOT(lockPressed()));
...
void
KeyFrameEditor::lockPressed()
{
  m_lockKeyframes = !m_lockKeyframes;
  if (m_lockKeyframes)
    m_lock->setIcon(QPixmap(":/images/lock.png"));
  else
    m_lock->setIcon(QPixmap(":/images/unlock.png"));
...
KeyFrameEditor::nextKeyFrame()
{
  ...
  m_vrKeyFrame = (m_vrKeyFrame+1)%m_fno.count();
  m_currFrame = m_fno[m_vrKeyFrame];
  emit playFrameNumber(m_currFrame);
}
void
KeyFrameEditor::prevKeyFrame()
{
  if (m_fno.count() == 0)
    return;
  
  m_vrKeyFrame = (m_vrKeyFrame+m_fno.count()-1)%m_fno.count();
  m_currFrame = m_fno[m_vrKeyFrame];
  emit playFrameNumber(m_currFrame);
}
//---------
...
KeyFrameEditor::drawVisibleRange(QPainter *p)
{
  ...
  p->setPen(QPen(QColor(50, 50, 50), 10,
		 Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
  p->drawLine(QPoint(5,5), QPoint(bg-10, 7));
  ...
  if (m_selectRegion.valid)
    {
      ...
      p->setPen(QPen(QColor(100, 250, 150), 5,
		     Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
  ...
  p->setPen(QPen(QColor(250, 125, 55), 5,
		 Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
			      ...
			      int fno, QRect rect, QImage img)
{
  ...
  p->drawImage(rect, img, QRect(0, 0, m_imgSize, m_imgSize));

  p->setBrush(Qt::transparent);
  p->setPen(QPen(penColor, 7,
		 Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
  p->drawRoundedRect(rect, m_imgSize/2, m_imgSize/2);
...
{
  m_p1 = QPoint(size().width()-10-m_imgSize/2, m_lineHeight);
  calcMaxFrame();
}
...
{
  // do not allow any manipulation when in Prayog Mode
  if (m_PrayogMode)
    return;
...
KeyFrameEditor::setImage(int fno, QImage img)
{
  ...
  QImage tmpimg = img.scaled(m_imgSize, m_imgSize);
  QImage cimg = QImage(m_imgSize, m_imgSize, QImage::Format_ARGB32);
  cimg.fill(255);
  QPainter imgP(&cimg);  
  QBrush b = QBrush(tmpimg);
  imgP.setBrush(b);
  imgP.drawEllipse(0,0, m_imgSize, m_imgSize);
  m_fImage[fno] = cimg;
...
void
KeyFrameEditor::newKeyframeNumber(int kfn, int fn)
{
  if (fn > 0)
    {
      m_fno[kfn] = fn;
...
void
KeyFrameEditor::removeAllKeyFrames()
{
  emit removeKeyFrames(0, m_fno.count()-1);

  int f0 = 0;
  int f1 = m_fno.count()-1;
			      ...
			      QList<QImage> images)
{
  ...
  for(int i=0; i<framenumbers.size(); i++)
  {
    QImage img = m_fImage[i].scaled(m_imgSize, m_imgSize);
    QImage cimg = QImage(m_imgSize, m_imgSize, QImage::Format_ARGB32);
    cimg.fill(255);
    QPainter imgP(&cimg);  
    QBrush b = QBrush(img);
...
KeyFrameEditor::showHelp()
{
  ...
  vlist.clear();
  QFile helpFile(":/keyframeeditor.help"+Global::helpLanguage());
