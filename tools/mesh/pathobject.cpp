...
void
PathObject::setCaptionText(QString txt)
{
  m_captionText = txt;
...
void
PathObject::setCaptionColor(QColor color)
{
...
void
PathObject::setCaptionFont(QFont font)
{
  m_captionFont = font;
  loadCaption(m_captionText,
	      m_captionFont,
...
void
PathObject::setCaption(QString ct, QFont cf,
...
PathObject::resetCaption()
{
  ...
  m_captionLabel = true;
  m_captionText.clear();
  m_captionFont = QFont("Arial", 48);
  m_captionColor = Qt::white;
  m_captionHaloColor = Qt::transparent;
...
//		    m_captionHaloColor.red());
    QColor penColor = m_captionHaloColor;
    // do not use alpha(),
...
//					m_imageName).absoluteFilePath();
  QString absoluteImageFile = imgFile;
  //----------------
...
PathObject::PathObject()
{
  ...
  m_sections = 4;
  m_segments = 1;
  m_color = Vec(0.7,0.7,0.7);
  m_lengthColor = Vec(0.7,0.7,0.7);
  m_opacity = 1;
  m_arrowHeadLength = 10;
  //m_lengthTextDistance = 10;
  m_lengthTextDistance = 0;
  m_points.clear();
  m_pointRadX.clear();
  m_pointRadY.clear();
  m_pointAngle.clear();
...
void PathObject::setLengthFlag(bool a) { m_lengthFlag = a; }
bool PathObject::lengthFlag() { return m_lengthFlag; }
...
void
PathObject::setRedAxisTarget(Vec targetRed)
{
//  // take project on x and y
//  Vec projX, projY, target;
//  projX = m_xaxis[0]*(targetRed*m_xaxis[0]);
//  projY = m_yaxis[0]*(targetRed*m_yaxis[0]);
//  target = (projX+projY).unit();
//
//  Vec axis;
//  float angle;
//  StaticFunctions::getRotationBetweenVectors(m_xaxis[0], target, axis, angle); 
//  angle = RAD2DEG(angle);
//  for(int j=0; j<m_points.count(); j++)
//    m_pointAngle[j] = angle;
//  m_updateFlag = true;
}
...
PathObject::computeTangents()
{
  ...
  if (nkf > 2)
    {
      Vec v0 = (m_points[0]-m_points[1]).unit();
      Vec v1 = (m_points[2]-m_points[1]).unit();
      pxaxis = v0^v1;
      ptang = -v0;
    }
			 ...
			 int x, int y)
{
  ...
  if (m_lengthFlag)
    {
      float len = length();
      str = QString("%1").arg(len, 0, 'f', Global::floatPrecision());
    }
  else
    {
      str = m_captionText;
...
PathObject::save(fstream& fout)
{
  ...
  memset(keyword, 0, 100);
  sprintf(keyword, "lengthflag");
  fout.write((char*)keyword, strlen(keyword)+1);
  fout.write((char*)&m_lengthFlag, sizeof(bool));
  ...
  while (!done)
    {
      ...
      else if (strcmp(keyword, "lengthflag") == 0)
	fin.read((char*)&m_lengthFlag, sizeof(bool));
