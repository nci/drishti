...
//	  this, SLOT(helpItemSelected(int)));
  ...
  connect(m_helpList, SIGNAL(itemClicked(QListWidgetItem*)),
		    ...
		    QStringList icons)
{
  ...
  gvbox->addLayout(gridLayout);
  gvbox->addStretch();
      
  QWidget *viewport = new QWidget;
  //viewport->setLayout(gridLayout);
  viewport->setLayout(gvbox);
  ...
  resize(600, 300+qBound(1, keys.count(), 15)*50);
  ...
  for(int i=0; i<keys.count(); i++)
      ...
      else
	{
	  ...
	  gridLayout->addWidget(lbl, i, 0, Qt::AlignRight | Qt::AlignVCenter);
	  ...
	  if (vlist[0] == "int")
	      ...
	      if (vlist.count() > 5)
		{
		  ...
		  for (int ii=5; ii<vlist.count(); ii++)
		    {		       
		      QStringList slst = vlist[ii].toString().split(',', QString::SkipEmptyParts);
		      QLabel *sl;
		      if (slst.count() > 1)
			{
	  ...
	  else if (vlist[0] == "font")
	    {
	      QPushButton *but = new QPushButton("Font");
	      gridLayout->addWidget(but, i, 1);
	      
	      m_signalMapper->setMapping(but, keys[i]);
  ...
  for(int i=0; i<keys.count(); i++)
      ...
      if (vlist.count() > 0)
	{
	  if (vlist[0] == "int")
	    ...
	    {
	      vmap[keys[i]] = qMakePair(vlist[1], true);
	    }
	}
    }
...
void
PropertyEditor::changeFont(QString key)
{
  QVariantList vlist = m_plist.value(key);
  QFont pfont = vlist[1].value<QFont>();
  bool ok;
  QFont font = QFontDialog::getFont(&ok, pfont, this);
  if (ok)
    {
      QVariantList vlist;
      vlist.clear();
...
void
PropertyEditor::itemClicked(QListWidgetItem *item)
{
  int row = m_helpList->row(item);
  if (m_prevRow != row)
    helpItemSelected(row);
  else
    {
      row = -1;
      m_helpList->setCurrentRow(-1);
      m_helpLabel->setText("");
      ui.lineEdit->setText("");
    }
