 ...
 * but WITHuint32_t ANY WARRANTY; without even the implied warranty of
...
public:
  ...
  bool find (uint32_t p, uint32_t q) {
  ...
  void unify (uint32_t p, uint32_t q) {
    if (p == q) {
      return;
    }
...
uint32_t* relabel(
    uint32_t* out_labels, const int64_t sx, const int64_t sy, const int64_t sz,
    const int64_t num_labels, DisjointSet &equivalences,
  ...
  uint32_t label;
  std::unique_ptr<uint32_t[]> renumber(new uint32_t[num_labels + 1]());
...
uint32_t* connected_components3d_26(
  ...
  max_labels = std::min(max_labels, static_cast<size_t>(std::numeric_limits<uint32_t>::max()));
  
  DisjointSet equivalences(max_labels);
  ...
  */
  ...
  return relabel(out_labels, sx, sy, sz, next_label, equivalences, N, runs.get());
}
...
uint32_t* connected_components2d_4(
  ...
  */
  ...
  return relabel(out_labels, sx, sy, /*sz=*/1, next_label, equivalences, N, runs.get());
}
...
uint32_t* connected_components3d(
  ...
  if (connectivity == 26) {
    ...
    return connected_components3d_18(
      in_labels, sx, sy, sz, 
      max_labels, out_labels, N
    );
  }
  else if (connectivity == 6) {
    return connected_components3d_6(
      in_labels, sx, sy, sz, 
    ...
    return connected_components2d_8(
      in_labels, sx, sy,
      max_labels, out_labels, N, periodic_boundary
    );
  }
    ...
    return connected_components2d_4(
      in_labels, sx, sy, 
      max_labels, out_labels, N, periodic_boundary
    );
  }
