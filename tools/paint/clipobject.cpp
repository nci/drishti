...
{
  Quaternion q(axis, qDegreesToRadians(angle));
...
ClipObject::processCommand(QString cmd)
{
  ...
  if (list[0] == "mop")
      ...
      if (list[0] == "rotate" || list[0] == "addrotation")
	{
	  ...
	  rot = Quaternion(Vec(x,y,z), qDegreesToRadians(a));
	}
      else
	{
	  float a=0;
	  ...
	  if (list[0] == "rotatex" || list[0] == "addrotationx")
	    rot = Quaternion(Vec(1,0,0), qDegreesToRadians(a));
	  else if (list[0] == "rotatey" || list[0] == "addrotationy")
	    rot = Quaternion(Vec(0,1,0), qDegreesToRadians(a));
	  else if (list[0] == "rotatez" || list[0] == "addrotationz")
	    rot = Quaternion(Vec(0,0,1), qDegreesToRadians(a));
	}
...
ClipObject::commandEditor()
{
  ...
  mesg += QString("rotation : %1 %2 %3 : %4\n").			\
    arg(axis.x).arg(axis.y).arg(axis.z).arg(qRadiansToDegrees(angle));
